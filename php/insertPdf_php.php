...
*/
 ...
 $getData=new getDataConnect();
   ...
   // gestion de la photo
 	if (isset($_FILES['insertPdf']) and !empty($_FILES['insertPdf'])) {
 		  ...
 		  $newPhoto->insertPdf($_FILES['photo']);
 	}
 ...
 if (isset($_POST['titre'],$_POST['description'],$_POST['idEnregistrement'],$_POST['idCour']) and !empty($_POST['titre']) and !empty($_POST['description']) and !empty($_POST['idEnregistrement']) and !empty($_POST['idCour'])) {
          ...
          $idCour=intval($_POST['idCour']);
         	...
         	if ($longueurTitre>10 and $longueurTitre <60) {
         	
	        	$newAdd=new AddCour($titre,$description);
	        	$newAdd->insertPdfElement($_SESSION['prof']['id'],$idEnregistrement,$idCour);
...
//ici on gere l'insertion des pdf

 // gestion de la photo
  if (isset($_FILES['photoPdf']) and !empty($_FILES['photoPdf'])) {
    
      
      $newPhoto=new AddCour("titre","description");
      $newPhoto->insertPhoto($_FILES['photo'],'ict_addPdf');
  }
